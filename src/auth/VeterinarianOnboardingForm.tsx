...
const VeterinarianOnboardingForm: React.FC<OnboardingFormProps> = ({ user, onComplete }) => {
    ...
    const removeClinic = (index: number) => {
        if (formData.clinics.length <= 1) return;
    ...
    const AutocompleteInput: React.FC<{ clinic: Partial<Clinic>, index: number, onClinicChange: (index: number, e: ChangeEvent<HTMLInputElement>) => void, onPlaceSelected: (index: number, place: GooglePlace) => void, isMapsReady: boolean }> = ({ clinic, index, onClinicChange, onPlaceSelected, isMapsReady }) => {
        ...
        return <Input ref={inputRef} label={`Clinic Name #${index + 1}`} name="clinic_name" value={clinic.clinic_name || ''} onChange={e => onClinicChange(index, e as ChangeEvent<HTMLInputElement>)} placeholder="Start typing clinic name..." />;
    };
                        ...
                        {isMapsScriptLoaded && formData.clinics.map((clinic, index) => {
                           ...
                           return (
                            <div key={index} className="p-4 border border-gray-200 dark:border-gray-700 rounded-md relative space-y-4 mb-4 bg-gray-50 dark:bg-gray-800/50">
                                ...
                                <AutocompleteInput clinic={clinic} index={index} onClinicChange={handleClinicChange as any} onPlaceSelected={handlePlaceSelected} isMapsReady={isMapsScriptLoaded} />
                                ...
                                {placeSelected && (
                                    <button type="button" onClick={() => clearPlaceSelection(index)} className="text-sm text-blue-600 hover:underline dark:text-blue-400">Clear & Enter Manually</button>
                                )}
                            </div>
                           )
                        })}
